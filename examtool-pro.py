...
def topic_search(tp):
    ...
    try:
        with open(filename, 'r') as file:
            topic = json.loads(file.read())
    except FileNotFoundError:
        return f'错误：文件”{file}“不存在！'
    except UnicodeDecodeError:
        return f'错误：文件”{file}“解码失败！'
    except json.decoder.JSONDecodeError:
        return f'错误：文件”{file}“不是标准的Json文件！'
    # 清空结果
    result = ''
    for item in topic:
        # 如果在题目中找到结果
...
class ExamToolPro(wx.Frame):
    def __init__(self, parent, uid):
        wx.Frame.__init__(self, parent, uid, 'ExamToolPro', pos=wx.DefaultPosition, size=(640, 480),
        ...
        self.file_input = wx.TextCtrl(panel, pos=(60, 20), size=(450, 25))
        self.file_input.SetValue('example.json')
    ...
    def file_loading(self, event):
        global filename
        filename = self.file_input.GetValue()
    ...
    def how_to_use(self, event):
        ...
        introduction = '用Python写的搜题软件，支持剪贴板一键复制和手动搜题功能'
        attention = '本软件采用GPL v3协议开源，可以自行修改源代码进行二次发布，但是二次发布的代码必须开源并注明原作者，如因使用本APP而产生的一切问题，作者不承担任何责任'
